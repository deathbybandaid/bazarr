...
def get_series_episodes(series_directory):
    ...
    for root, dirs, files in os.walk(series_directory):
        for filename in files:
            if (
                os.path.splitext(filename)[1] in VIDEO_EXTENSION
                and filename[0] != "."
            ):
...
def get_episode_metadata(file, tmdbid, series_id, update=False):
    ...
    if "season" in guessed and "episode" in guessed:
        ...
        try:
            ...
            logging.exception(
                f"BAZARR is facing issues indexing this episodes: {file}"
            )
            return False
        else:
            episode_metadata = {
...
def update_series_episodes(seriesId=None, use_cache=True):
    ...
    if seriesId:
        ...
        series_ids = [
            x["seriesId"]
            for x in TableShows.select(TableShows.seriesId).dicts()
        ]

    for series_id in series_ids:
        # getting episodes from db for that series
        existing_series_episodes = (
            ...
            # update existing episodes metadata
                ...
                if episode_metadata:
                    ...
                    TableEpisodes.update(episode_metadata).where(
                        TableEpisodes.episodeId
                        == existing_series_episode["episodeId"]
                    ).execute()
                    # indexing existing subtitles and missing ones.
                    store_subtitles(
                        existing_series_episode["path"], use_cache=use_cache
                    )
        ...
        try:
            ...
            series_metadata = (
                TableShows.select(TableShows.path, TableShows.tmdbId)
        ...
        for episode in episodes:
            if episode in existing_episodes:
                ...
                episode_metadata = get_episode_metadata(
                    episode, series_metadata["tmdbId"], series_id, update=False
                )
                if episode_metadata:
                    try:
                        # insert episod eto db
                        result = TableEpisodes.insert(
                            episode_metadata
                        ).execute()
                    except Exception as e:
                        logging.error(
                            f"BAZARR is unable to insert this episode to the database: "
