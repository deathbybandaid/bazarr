...
def list_movies_directories(root_dir_id):
    ...
    try:
        # get root folder row
        root_dir_path = (
            TableMoviesRootfolder.select(TableMoviesRootfolder.path)
        ...
        if not root_dir_path:
            logging.debug(
                f"BAZARR cannot find the specified movies root folder: {root_dir_id}"
            )
...
def get_movies_match(directory):
    ...
    try:
        ...
        logging.exception(
            "BAZARR is facing issues indexing movies: {0}".format(repr(e))
        )
    else:
        matching_movies = []
    ...
    if tmdbid:
        try:
            ...
            alternative_titles = tmdb_func_cache(
                tmdb.Movies(tmdbid).alternative_titles
            )
            ...
            movies_metadata = {
                ...
                "year": movies_info["release_date"][:4]
                if movies_info["release_date"]
                else None,
                "overview": movies_info["overview"],
                "poster": images_url.format(movies_info["poster_path"])
                if movies_info["poster_path"]
                else None,
                "fanart": images_url.format(movies_info["backdrop_path"])
                if movies_info["backdrop_path"]
                else None,
                "alternativeTitles": [
                    x["title"] for x in alternative_titles["titles"]
                ],
            ...
            if dir_name:
                ...
                movies_metadata["path"] = (
                    os.path.join(movie_dir, movie_file) if movie_file else None
                )
                movies_metadata["tmdbId"] = tmdbid

                if movie_file:
                    movies_metadata.update(
                        video_prop_reader(os.path.join(movie_dir, movie_file))
                    )
            else:
...
def update_indexed_movies():
    ...
    root_dir_ids = TableMoviesRootfolder.select(
        TableMoviesRootfolder.rootId, TableMoviesRootfolder.path
    ).dicts()
    for root_dir_id in root_dir_ids:
        # for each root folder, get the existing movies rows
        existing_movies = (
            TableMovies.select(
                TableMovies.path, TableMovies.movieId, TableMovies.tmdbId
            )
        ...
        for existing_movie in existing_movies:
            ...
            if not os.path.exists(existing_movie["path"]):
                TableMovies.delete().where(
                    TableMovies.path == existing_movie["path"]
                ).execute()
            # update existing episodes metadata
            else:
                movie_metadata = get_movies_metadata(
                    tmdbid=existing_movie["tmdbId"],
                    root_dir_id=root_dir_id["rootId"],
                    movie_path=existing_movie["path"],
                )
                if movie_metadata:
                    TableMovies.update(movie_metadata).where(
                        TableMovies.movieId == existing_movie["movieId"]
                    ).execute()
                    store_subtitles_movie(
                        existing_movie["path"], use_cache=True
                    )

        # add missing movies to database
        root_dir_subdirectories = list_movies_directories(
            root_dir_id["rootId"]
        )
        # get existing movies paths
        existing_movies_paths = [
            os.path.dirname(x["path"]) for x in existing_movies
        ]
        for root_dir_subdirectory in root_dir_subdirectories:
            if (
                ...
                if root_dir_match:
                    ...
                    directory_metadata = get_movies_metadata(
                    ...
                    if directory_metadata and directory_metadata["path"]:
                        try:
                            ...
                            result = TableMovies.insert(
                                directory_metadata
                            ).execute()
                        except Exception as e:
                            logging.error(
                                f"BAZARR is unable to insert this movie to the database: "
                            ...
                            if result:
                                ...
                                store_subtitles_movie(
                                    directory_metadata["path"], use_cache=False
                                )
...
def update_specific_movie(movieId, use_cache=True):
    ...
    if directory_metadata:
        try:
            # update the db
            result = (
                TableMovies.update(directory_metadata)
                .where(TableMovies.movieId == movieId)
                .execute()
            )
        except Exception as e:
            logging.error(
                f"BAZARR is unable to update this movie to the database: "
            ...
            if result:
                ...
                store_subtitles_movie(
                    movie_metadata["path"], use_cache=use_cache
                )
