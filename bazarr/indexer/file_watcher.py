...
class FileWatcher:
    def __init__(self):
        ...
        self.fs_event_handler = PatternMatchingEventHandler(
            self.patterns,
        ...
        try:
            ...
            self.timeout = 60
            logging.debug(
                f"BAZARR file watcher is using the default interval of {self.timeout} seconds."
            )
        else:
            logging.debug(
                f"BAZARR file watcher is using the configured interval of {self.timeout} seconds."
            )
            ...
            if isinstance(root_dir, TableShowsRootfolder):
                if event.event_type == "deleted":
                    ...
                    TableEpisodes.delete().where(
                        TableEpisodes.path == event.src_path
                    ).execute()
                elif event.event_type in ["created", "modified"]:
                    # add or update existing episode
                    series_metadata = self.get_series_from_episode_path(
                        event.src_path
                    )
                    ...
                    if series_metadata:
                        ...
                        if episode_metadata:
                            if event.event_type == "created":
                                ...
                                TableEpisodes.update(episode_metadata).where(
                                    TableEpisodes.path == event.src_path
                                ).execute()
    ...
    def get_series_from_episode_path(path):
        ...
        try:
            series_metadata = (
                TableShows.select()
                .where(TableShows.path == series_dir)
                .dicts()
                .get()
            )
    ...
    def config(self):
        ...
        if settings.general.getboolean("use_series"):
            self.series_directories = [
                x["path"] for x in TableShowsRootfolder.select().dicts()
            ]
            for series_directory in self.series_directories:
                if os.path.exists(series_directory):
                    self.series_observer.schedule(
                        self.fs_event_handler, series_directory, recursive=True
                    )
                    logging.debug(
                        f"BAZARR is watching this series root directory: {series_directory}"
                    )
                else:
                    logging.error(
                        f"BAZARR cannot access this series root directory: {series_directory}"
                    )
        if settings.general.getboolean("use_movies"):
            self.movies_directories = [
                x["path"] for x in TableMoviesRootfolder.select().dicts()
            ]
            for movies_directory in self.movies_directories:
                if os.path.exists(movies_directory):
                    self.movies_observer.schedule(
                        self.fs_event_handler, movies_directory, recursive=True
                    )
                    logging.debug(
                        f"BAZARR is watching this movies root directory: {movies_directory}"
                    )
                else:
                    logging.error(
                        f"BAZARR cannot access this movies root directory: {movies_directory}"
                    )
