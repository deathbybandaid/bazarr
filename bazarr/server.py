...
class Server:
    ...
    def start(self):
        try:
            logging.info(
                "BAZARR is started and waiting for request on http://"
                + str(settings.general.ip)
                + ":"
                + (str(args.port) if args.port else str(settings.general.port))
                + str(base_url)
            )
    ...
    def shutdown(self):
        try:
            ...
            try:
                stop_file = io.open(
                    os.path.join(args.config_dir, "bazarr.stop"),
                    "w",
                    encoding="UTF-8",
                )
            except Exception as e:
                logging.error(
                    "BAZARR Cannot create bazarr.stop file: " + repr(e)
                )
            else:
