...
from subliminal_patch.core import (
...
def get_video(path, title, providers=None, media_type="movie"):
    ...
    try:
        video = parse_video(
            path, hints=hints, providers=providers, hash_from=hash_from
        )
        ...
        logging.debug(
            "BAZARR is using these video object properties: %s",
            vars(copy.deepcopy(video)),
        )
        return video
...
def download_subtitle(
    ...
    """
    video = get_video(
        force_unicode(path), title, providers=providers, media_type=media_type
    )
    if video:
        handler = series_score if media_type == "series" else movie_score
        min_score, max_score, scores = _get_scores(
            media_type, minimum_score_movie, minimum_score
        )
            ...
            for video, subtitles in downloaded_subtitles.items():
                ...
                try:
                    ...
                    chmod = (
                        ...
                        and settings.general.getboolean("chmod_enabled")
                        else None
                    )
                    ...
                    for subtitle in saved_subtitles:
                        ...
                        downloaded_language_code3 = _get_download_code3(
                            subtitle
                        )
                        ...
                        audio_language_code2 = alpha2_from_language(
                            audio_language
                        )
                        audio_language_code3 = alpha3_from_language(
                            audio_language
                        )
                        ...
                        if is_upgrade:
                            ...
                            action = "downloaded"
                        percent_score = round(
                            subtitle.score * 100 / max_score, 2
                        )
                        message = (
                            downloaded_language
                            + modifier_string
                        ...
                        if media_type == "series":
                            ...
                            movie_metadata = (
                                TableMovies.select(TableMovies.movieId)
                        ...
                        if use_postprocessing is True:
                            ...
                            if not use_pp_threshold or (
                                use_pp_threshold
                                and percent_score < pp_threshold
                            ):
                                logging.debug(
                                    "BAZARR Using post-processing command: {}".format(
                                        command
                                    )
                                )
                        ...
                        if media_type == "series":
                            event_stream(
                                type="episode-wanted",
                                action="delete",
                                payload=episode_metadata["episodeId"],
                            )
                            ...
                            event_stream(
                                type="movie-wanted",
                                action="delete",
                                payload=movie_metadata["movieId"],
                            )

                        track_event(
                            category=downloaded_provider,
                            action=action,
        ...
        if not saved_any:
            logging.debug(
                "BAZARR No Subtitles were found for this file: " + path
            )
            return None
...
def manual_search(
    path, profileId, providers, providers_auth, title, media_type
):
        ...
        try:
            ...
            for s in subtitles[video]:
                ...
                for language in initial_language_set:
                    if (
                        s.language.basename == language.basename
                        and s.language.forced == language.forced
                        and s.language.hi == language.hi
                    ):
                ...
                subtitles_list.append(
                    dict(
                        ...
                        subtitle=codecs.encode(
                            pickle.dumps(s.make_picklable()), "base64"
                        ).decode(),
                        url=s.page_link,
            ...
            logging.debug(
                "BAZARR "
                + str(len(final_subtitles))
                + " Subtitles have been found for this file: "
                + path
            )
            logging.debug(
                "BAZARR Ended searching Subtitles for this file: " + path
            )
...
def manual_download_subtitle(
    ...
    logging.debug(
        "BAZARR Manually downloading Subtitles for this file: " + path
    )
    ...
    single = settings.general.getboolean("single_language")
    video = get_video(
        force_unicode(path), title, providers={provider}, media_type=media_type
    )
    if video:
    ...
    logging.debug(
        "BAZARR Ended manually downloading Subtitles for file: " + path
    )


def manual_upload_subtitle(
    path, language, forced, hi, title, media_type, subtitle, audio_language
):
    ...
    sub = Subtitle(
        lang_obj, mods=get_array_from(settings.general.subzero_mods)
    )
    ...
    if len(saved_subtitles) < 1:
        logging.exception(
            "BAZARR Error saving Subtitles file to disk for this file:" + path
        )
        return
    ...
    if media_type == "series":
        episode_metadata = (
            TableEpisodes.select(
                TableEpisodes.seriesId, TableEpisodes.episodeId
            )
            .where(TableEpisodes.path == path)
            .dicts()
            .get()
        ...
        sync_subtitles(
            ...
            srt_lang=uploaded_language_code2,
            media_type=media_type,
            percent_score=100,
            series_id=episode_metadata["seriesId"],
            episode_id=episode_metadata["episodeId"],
    ...
    if use_postprocessing:
        command = pp_replace(
            ...
            subtitle_path,
            uploaded_language,
            uploaded_language_code2,
            uploaded_language_code3,
            audio_language,
            audio_language_code2,
            audio_language_code3,
            forced,
...
def series_download_subtitles(no):
    ...
    if not episodes_details:
        logging.debug(
            "BAZARR no episode for that seriesId have been found in database or they have all been "
            "ignored because of monitored status, series type or series tags: {}".format(
                no
            )
        )
        return
    ...
    for i, episode in enumerate(episodes_details):
        if providers_list:
            show_progress(
                id="series_search_progress_{}".format(no),
            ...
            for language in ast.literal_eval(episode["missing_subtitles"]):
                ...
                confirmed_missing_subs = (
                    TableEpisodes.select(TableEpisodes.missing_subtitles)
...
def episode_download_subtitles(no, send_progress=False):
    ...
    if not episodes_details:
        logging.debug(
            "BAZARR no episode with that episodeId can be found in database:",
            str(no),
        )
        return
    ...
    for episode in episodes_details:
        if providers_list:
            if send_progress:
                show_progress(
                    id="episode_search_progress_{}".format(no),
                    header="Searching missing subtitles...",
                    name="{0} - S{1:02d}E{2:02d} - {3}".format(
                        episode["title"],
                        episode["season"],
            ...
            for language in ast.literal_eval(episode["missing_subtitles"]):
                ...
                if language is not None:
                    ...
                    if result is not None:
                        ...
                        send_notifications(
                            episode["seriesId"], episode["episodeId"], message
                        )
            if send_progress:
...
def movies_download_subtitles(no):
    ...
    if not len(movies):
        logging.debug(
            "BAZARR no movie with that movieId can be found in database:",
            str(no),
        )
        return
    else:
    ...
    for i, language in enumerate(ast.literal_eval(movie["missing_subtitles"])):
        # confirm if language is still missing or if cutoff have been reached
        confirmed_missing_subs = (
            TableMovies.select(TableMovies.missing_subtitles)
        ...
        if providers_list:
            show_progress(
                id="movie_search_progress_{}".format(no),
                header="Searching missing subtitles...",
                name=movie["title"],
                value=i,
                count=count_movie,
    ...
    for episode in episodes_details:
        ...
        for language in ast.literal_eval(episode["missing_subtitles"]):
            ...
            TableEpisodes.update(
                {TableEpisodes.failedAttempts: str(attempt)}
            ).where(TableEpisodes.episodeId == episode["episodeId"]).execute()
...
def wanted_download_subtitles_movie(movie_id):
    ...
    for movie in movies_details:
        ...
        for language in ast.literal_eval(movie["missing_subtitles"]):
            ...
            TableMovies.update(
                {TableMovies.failedAttempts: str(attempt)}
            ).where(TableMovies.movieId == movie["movieId"]).execute()
            ...
            for i in range(len(attempt)):
                if attempt[i][0] == language:
                    if search_active(attempt[i][1]) is True:
                        ...
                        logging.info(
                            "BAZARR Search is not active for this Movie "
                            + movie["path"]
                            + " Language: "
                            + attempt[i][0]
                        )
...
def search_active(timestamp):
    if settings.general.getboolean("adaptive_searching"):
        ...
        attempt_datetime = datetime.strptime(
            str(aa).split(".")[0], "%Y-%m-%d %H:%M:%S"
        )
        attempt_search_deadline = attempt_datetime + search_deadline
        today = datetime.today()
        attempt_age_in_days = (
            today.date() - attempt_search_deadline.date()
        ).days
...
def refine_from_db(path, video):
    if isinstance(video, Episode):
        ...
        if len(data):
            ...
            video.alternative_titles = ast.literal_eval(
...
def refine_from_ffprobe(path, video):
    if isinstance(video, Movie):
        file_id = (
            TableMovies.select(TableMovies.file_size)
            .where(TableMovies.path == path)
            .dicts()
            .get()
        )
        ...
        file_id = (
            TableEpisodes.select(TableEpisodes.file_size)
            .where(TableEpisodes.path == path)
            .dicts()
            .get()
        )
    ...
    if isinstance(video, Movie):
        data = parse_video_metadata(
            file=path, file_size=file_id["file_size"], media_type="movie"
        )
    else:
        data = parse_video_metadata(
            file=path, file_size=file_id["file_size"], media_type="episode"
        )

    if not data["ffprobe"]:
        logging.debug(
            "No FFprobe available in cache for this file: {}".format(path)
        )
        return video
    ...
    if "video" not in data["ffprobe"]:
        logging.debug(
            "BAZARR FFprobe was unable to find video tracks in the file!"
        )
    else:
        ...
        if "frame_rate" in data["ffprobe"]["video"][0]:
            if not video.fps:
                if isinstance(
                    data["ffprobe"]["video"][0]["frame_rate"], float
                ):
                    video.fps = data["ffprobe"]["video"][0]["frame_rate"]
                else:
                    video.fps = data["ffprobe"]["video"][0][
                        "frame_rate"
                    ].magnitude

    if "audio" not in data["ffprobe"]:
        logging.debug(
            "BAZARR FFprobe was unable to find audio tracks in the file!"
        )
    else:
...
def upgrade_subtitles():
    ...
    if settings.general.getboolean("use_series"):
        upgradable_episodes_conditions = [
            (TableHistory.action << query_actions),
        ...
        for upgradable_episode in upgradable_episodes:
            if upgradable_episode["timestamp"] > minimum_timestamp:
                try:
                    ...
                    if int(upgradable_episode["score"]) < 360 or (
                        settings.general.getboolean("upgrade_manual")
                        and upgradable_episode["action"] in [2, 4, 6]
                    ):
        ...
        for episode in upgradable_episodes_not_perfect:
            if (
                os.path.exists(episode["subtitles_path"])
                and int(episode["score"]) < 357
            ):
    ...
    if settings.general.getboolean("use_movies"):
        ...
        for upgradable_movie in upgradable_movies:
            if upgradable_movie["timestamp"] > minimum_timestamp:
                try:
                    ...
                    if int(upgradable_movie["score"]) < 120 or (
                        settings.general.getboolean("upgrade_manual")
                        and upgradable_movie["action"] in [2, 4, 6]
                    ):
        ...
        for movie in upgradable_movies_not_perfect:
            if (
                os.path.exists(movie["subtitles_path"])
                and int(movie["score"]) < 117
            ):
    ...
    logging.info(
        "BAZARR Finished searching for Subtitles to upgrade. Check History for more information."
    )
...
def postprocessing(command, path):
    try:
        ...
        logging.error(
            "BAZARR Post-processing failed for file " + path + " : " + repr(e)
        )
    else:
        if out == "":
            logging.info(
            ...
            logging.info(
                "BAZARR Post-processing result for file " + path + " : " + out
            )
...
def sync_subtitles(
    ...
    if settings.subsync.getboolean("use_subsync"):
        if media_type == "series":
            use_subsync_threshold = settings.subsync.getboolean(
                "use_subsync_threshold"
            )
            subsync_threshold = settings.subsync.subsync_threshold
        else:
            use_subsync_threshold = settings.subsync.getboolean(
                "use_subsync_movie_threshold"
            )
            subsync_threshold = settings.subsync.subsync_movie_threshold

        if not use_subsync_threshold or (
            use_subsync_threshold and percent_score < float(subsync_threshold)
        ):
            subsync.sync(
            ...
            logging.debug(
                "BAZARR subsync skipped because subtitles score isn't below this "
