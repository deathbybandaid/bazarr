...
from indexer.series.local.series_indexer import (
    list_series_directories,
    get_series_match,
    get_series_metadata,
)
from indexer.movies.local.movies_indexer import (
    ...
    if "language" in item:
        if item["language"] == "None":
            ...
            item["language"] = {
                ...
                "forced": True
                if item["language"].endswith(":forced")
                else False,
...
def postprocessSeries(item):
    ...
    if "audio_language" in item and item["audio_language"] is not None:
        item["audio_language"] = get_audio_profile_languages(
    ...
    if "alternateTitles" in item:
        if item["alternateTitles"] is None:
            ...
            item["alternativeTitles"] = ast.literal_eval(
...
def postprocessEpisode(item):
    ...
    if "subtitles" in item:
        ...
        for subs in raw_subtitles:
            ...
            sub = {
                "name": language_from_alpha2(subtitle[0]),
                "code2": subtitle[0],
    ...
    if "missing_subtitles" in item:
        if item["missing_subtitles"] is None:
            ...
            item["missing_subtitles"] = ast.literal_eval(
                item["missing_subtitles"]
            )
...
def postprocessMovie(item):
    ...
    if "subtitles" in item:
        ...
        item["subtitles"] = sorted(
            item["subtitles"], key=itemgetter("name", "forced")
        )
...
class Badges(Resource):
    ...
    def get(self):
        episodes_conditions = [
            (TableEpisodes.missing_subtitles is not None),
            (TableEpisodes.missing_subtitles != "[]"),
        ]
        episodes_conditions += get_exclusion_clause("series")
        missing_episodes = (
            TableEpisodes.select(
                TableShows.tags, TableShows.seriesType, TableEpisodes.monitored
            )
            .join(TableShows)
        ...
        movies_conditions = [
            (TableMovies.missing_subtitles is not None),
            (TableMovies.missing_subtitles != "[]"),
        ]
        movies_conditions += get_exclusion_clause("movie")
        missing_movies = (
            TableMovies.select(TableMovies.tags, TableMovies.monitored)
...
class Languages(Resource):
    ...
    def get(self):
        ...
        if history and history not in False_Keys:
            ...
            languages_list = list(
                set([lang["language"].split(":")[0] for lang in languages])
            )
            languages_dicts = []
...
class Notifications(Resource):
    ...
    def patch(self):
        ...
        apobj.notify(
            title="Bazarr test notification", body="Test notification"
        )
    ...
    def get(self):
        ...
        if query:
            if settings.general.getboolean("use_series"):
                ...
                series = (
                    TableShows.select(
                        TableShows.title, TableShows.seriesId, TableShows.year
                    )
    ...
    def post(self):
        ...
        if languages_profiles:
            ...
            for item in json.loads(languages_profiles):
                if item["profileId"] in existing:
                    ...
                    TableLanguagesProfiles.update(
                        {
                            ...
                            TableLanguagesProfiles.cutoff: item["cutoff"]
                            if item["cutoff"] != "null"
                            else None,
                    ...
                    TableLanguagesProfiles.insert(
                        {
                            TableLanguagesProfiles.profileId: item[
                                "profileId"
                            ],
                            TableLanguagesProfiles.name: item["name"],
                            TableLanguagesProfiles.cutoff: item["cutoff"]
            ...
            for profileId in existing:
                ...
                TableShows.update({TableShows.profileId: None}).where(
                    TableShows.profileId == profileId
                ).execute()
                TableMovies.update({TableMovies.profileId: None}).where(
                    TableMovies.profileId == profileId
                ).execute()
                # Remove deleted profiles
                TableLanguagesProfiles.delete().where(
                    TableLanguagesProfiles.profileId == profileId
                ).execute()
            ...
            if settings.general.getboolean("use_series"):
                scheduler.add_job(
                    list_missing_subtitles, kwargs={"send_event": False}
                )
            if settings.general.getboolean("use_movies"):
                scheduler.add_job(
                    list_missing_subtitles_movies, kwargs={"send_event": False}
                )
...
class SystemStatus(Resource):
    ...
    def get(self):
        ...
        system_status.update(
            {"bazarr_directory": os.path.dirname(os.path.dirname(__file__))}
        )
...
class SystemReleases(Resource):
    ...
    def get(self):
        ...
        try:
            with io.open(
                os.path.join(args.config_dir, "config", "releases.txt"),
                "r",
                encoding="UTF-8",
            ) as f:
            ...
            for release in releases:
                if (
                    settings.general.branch == "master"
                    and not release["prerelease"]
                ):
                    filtered_releases.append(release)
                elif settings.general.branch != "master" and any(
            ...
            logging.exception(
                "BAZARR cannot parse releases caching file: "
                + os.path.join(args.config_dir, "config", "releases.txt")
            )
...
class Series(Resource):
    ...
    def get(self):
        ...
        for item in result:
            ...
            episodes_missing_conditions = [
                (TableEpisodes.seriesId == item["seriesId"]),
                (TableEpisodes.missing_subtitles != "[]"),
            ]
    ...
    def post(self):
        ...
        for idx in range(len(seriesIdList)):
            ...
            episode_id_list = (
                TableEpisodes.select(TableEpisodes.episodeId)
...
class Episodes(Resource):
    ...
    def get(self):
        ...
        if len(episodeId) > 0:
            result = (
                TableEpisodes.select()
                .where(TableEpisodes.episodeId.in_(episodeId))
                .dicts()
            )
        elif len(seriesId) > 0:
            result = (
    ...
    def post(self):
        ...
        return jsonify(
            data=list(
                ...
                .where(TableShowsRootfolder.rootId == result)
                .dicts()
            )
        )
...
class SeriesAdd(Resource):
    ...
    def post(self):
        ...
        series_metadata = get_series_metadata(
            tmdbid=tmdbId, root_dir_id=rootdir_id, dir_name=directory
        )
...
class Movies(Resource):
    ...
    def post(self):
        ...
        for idx in range(len(movieIdList)):
            ...
            TableMovies.update({TableMovies.profileId: profileId}).where(
                TableMovies.movieId == movieId
            ).execute()
...
class MoviesSubtitles(Resource):
    ...
    def patch(self):
        ...
        movieInfo = (
            TableMovies.select(
                TableMovies.title, TableMovies.path, TableMovies.audio_language
            )
    ...
    def post(self):
        ...
        return jsonify(
            data=list(
                ...
                .where(TableMoviesRootfolder.rootId == result)
                .dicts()
            )
        )
...
class Providers(Resource):
    ...
    def get(self):
        ...
        if history and history not in False_Keys:
            providers = list(
                TableHistory.select(TableHistory.provider)
                .where(
                    TableHistory.provider is not None
                    and TableHistory.provider != "manual"
                )
                .dicts()
            )
            providers += list(
                TableHistoryMovie.select(TableHistoryMovie.provider)
                .where(
                    TableHistoryMovie.provider is not None
...
class ProviderMovies(Resource):
    ...
    def get(self):
        ...
        movieInfo = (
            TableMovies.select(
                TableMovies.title, TableMovies.path, TableMovies.profileId
            )
            .where(TableMovies.movieId == movieId)
            .dicts()
            .get()
...
class ProviderEpisodes(Resource):
    ...
    def get(self):
        ...
        episodeInfo = (
            TableEpisodes.select(
                TableEpisodes.title, TableEpisodes.path, TableShows.profileId
            )
            .join(TableShows)
            .where(TableEpisodes.episodeId == episodeId)
            .dicts()
    ...
    def post(self):
        ...
        episodeId = request.args.get("episodeid")
        episodeInfo = (
            TableEpisodes.select(TableEpisodes.title, TableEpisodes.path)
            .where(TableEpisodes.episodeId == episodeId)
            .dicts()
            .get()
        )
...
class EpisodesHistory(Resource):
    ...
    def get(self):
        ...
        blacklist_db = TableBlacklist.select(
        ...
        for item in episode_history:
            ...
            if item["score"]:
                item["score"] = (
                    str(round((int(item["score"]) * 100 / 360), 2)) + "%"
                )
        ...
        count = (
            TableHistory.select()
            .join(TableEpisodes)
            .where(TableEpisodes.title is not None)
            .count()
        )
...
class MoviesHistory(Resource):
    ...
    def get(self):
        ...
        for item in movie_history:
            ...
            if item["score"]:
                item["score"] = (
                    str(round((int(item["score"]) * 100 / 120), 2)) + "%"
                )
...
class HistoryStats(Resource):
    ...
    def get(self):
        ...
        history_where_clause_movie = reduce(
            operator.and_, history_where_clauses_movie
        )

        data_series = (
            TableHistory.select(
                fn.strftime(
                    "%Y-%m-%d", TableHistory.timestamp, "unixepoch"
                ).alias("date"),
                fn.COUNT(TableHistory.id).alias("count"),
            )
            .where(history_where_clause)
            .group_by(
                fn.strftime("%Y-%m-%d", TableHistory.timestamp, "unixepoch")
            )
            .dicts()
        )
        ...
        data_movies = (
            TableHistoryMovie.select(
                fn.strftime(
                    "%Y-%m-%d", TableHistoryMovie.timestamp, "unixepoch"
                ).alias("date"),
        ...
        for dt in rrule.rrule(
            ...
            if not any(
                d["date"] == dt.strftime("%Y-%m-%d") for d in data_series
            ):
                data_series.append(
                    {"date": dt.strftime("%Y-%m-%d"), "count": 0}
                )
            if not any(
                d["date"] == dt.strftime("%Y-%m-%d") for d in data_movies
            ):
                data_movies.append(
                    {"date": dt.strftime("%Y-%m-%d"), "count": 0}
                )
...
class MoviesWanted(Resource):
    ...
    def get(self):
        ...
        count = (
            TableMovies.select(TableMovies.monitored, TableMovies.tags)
            .where(reduce(operator.and_, count_conditions))
            .count()
        )
...
class EpisodesBlacklist(Resource):
    ...
    def post(self):
        ...
        episodeInfo = (
            TableEpisodes.select(TableEpisodes.path)
...
class Subtitles(Resource):
    ...
    def patch(self):
        ...
        if media_type == "episode":
            metadata = (
                TableEpisodes.select(
                    TableEpisodes.path, TableEpisodes.seriesId
                )
    ...
    def get(self):
        ...
        for name in names:
            ...
            if "subtitle_language" in guessit_result:
                result["subtitle_language"] = str(
                    guessit_result["subtitle_language"]
                )
        ...
        if media_type == "episode":
            try:
                ...
                episodeId = (
                    ...
                    .join(TableShows)
                    .where(
                        TableShows.imdbId == series_imdb_id,
                ...
                if episodeId:
                    episode_download_subtitles(
                        no=episodeId["episodeId"], send_progress=True
                    )
        else:
            try:
