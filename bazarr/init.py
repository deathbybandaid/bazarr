...
def init():
    ...
    os.environ["SZ_USER_AGENT"] = "Bazarr/{}".format(
    ...
    if not args.no_update:
        try:
            ...
            try:
                ...
                logging.info(
                    "BAZARR unable to install requirements (pip not installed)."
                )
            else:
                if os.path.expanduser("~") == "/":
                    logging.info(
                        "BAZARR unable to install requirements (user without home directory)."
                    )
                else:
                    ...
                    try:
                        ...
                        subprocess.check_output(
                            pip_command, stderr=subprocess.STDOUT
                        )
                    except subprocess.CalledProcessError as e:
                        logging.exception(
                            "BAZARR requirements.txt installation result: {}".format(
                                e.stdout
                            )
                        )
                        os._exit(1)
                    ...
                    try:
                        restart_file = io.open(
                            os.path.join(args.config_dir, "bazarr.restart"),
                            "w",
                            encoding="UTF-8",
                        )
                    except Exception as e:
                        logging.error(
                            "BAZARR Cannot create bazarr.restart file: "
                            + repr(e)
                        )
                    else:
    ...
    if not settings.auth.apikey or settings.auth.apikey.startswith("b'"):
        ...
        with open(
            os.path.join(args.config_dir, "config", "config.ini"), "w+"
        ) as handle:
            ...
            with open(package_info_file) as file:
                ...
                for line in splitted_lines:
                    ...
                    if len(splitted_line) == 2:
                        package_info[splitted_line[0].lower()] = splitted_line[
                            1
                        ].replace("\n", "")
                    else:
                        continue
    ...
    if settings.analytics.visitor:
        with open(
            os.path.normpath(
                os.path.join(args.config_dir, "config", "analytics.dat")
            ),
            "w+",
        ) as handle:
