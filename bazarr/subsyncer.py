            ...
            if os.path.isfile(self.srtout):
                if not settings.subsync.getboolean("debug"):
                    ...
                    framerate_scale_factor = (
                        result["framerate_scale_factor"] or 0
                    )
                    message = (
                        "{0} subtitles synchronization ended with an offset of {1} seconds and a framerate "
                        "scale factor of {2}.".format(
                            language_from_alpha2(srt_lang),
                            offset_seconds,
                ...
                logging.error(
                    "BAZARR unable to sync subtitles: {0}".format(self.srtin)
                )
