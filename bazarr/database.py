...
)  # Run a checkpoint and merge remaining wal-journal.
...
class TableShows(BaseModel):
    ...
    rootdir = ForeignKeyField(
        TableShowsRootfolder, db_column="rootId", on_delete="CASCADE"
    )
...
class TableEpisodes(BaseModel):
    ...
    seriesId = ForeignKeyField(
        TableShows, db_column="seriesId", on_delete="CASCADE"
    )
...
class TableHistory(BaseModel):
    ...
    episodeId = ForeignKeyField(
        TableEpisodes, db_column="episodeId", on_delete="CASCADE"
    )
    seriesId = ForeignKeyField(
        TableShows, db_column="seriesId", on_delete="CASCADE"
    )
...
class TableBlacklist(BaseModel):
    ...
    episode_id = ForeignKeyField(
        TableEpisodes, db_column="episodeId", on_delete="CASCADE"
    )
    series_id = ForeignKeyField(
        TableShows, db_column="seriesId", on_delete="CASCADE"
    )
...
class TableMovies(BaseModel):
    ...
    year = TextField(null=True)
    rootdir = ForeignKeyField(
        TableMoviesRootfolder, db_column="rootId", on_delete="CASCADE"
    )
...
class TableHistoryMovie(BaseModel):
    ...
    language = TextField(null=True)
    provider = TextField(null=True)
    movieId = ForeignKeyField(
        TableMovies, db_column="movieId", on_delete="CASCADE"
    )
    score = TextField(null=True)
    subs_id = TextField(null=True)
    subtitles_path = TextField(null=True)
    ...
    while not tables_created:
        try:
            if not System.select().count():
                System.insert(
                    {System.configured: "0", System.updated: "0"}
                ).execute()
        except Exception:
            gevent.sleep(0.1)
        else:
...
def get_audio_profile_languages(
    series_id=None, episode_id=None, movie_id=None
):
    ...
    if series_id:
        audio_languages_list_str = TableShows.get(
            TableShows.seriesId == series_id
        ).audio_language
    elif episode_id:
        audio_languages_list_str = TableEpisodes.get(
            TableEpisodes.episodeId == episode_id
        ).audio_language
    elif movie_id:
        audio_languages_list_str = TableMovies.get(
            TableMovies.movieId == movie_id
        ).audio_language
    else:
