...
def check_releases():
    releases = []
    url_releases = (
        "https://api.github.com/repos/Bazarr/Bazarr2/releases?per_page=100"
    )
    try:
        logging.debug(
            "BAZARR getting releases from Github: {}".format(url_releases)
        )
        ...
        with open(
            os.path.join(args.config_dir, "config", "releases.txt"), "w"
        ) as f:
            json.dump(releases, f)
        logging.debug(
            "BAZARR saved {} releases to releases.txt".format(len(r.json()))
        )
...
def check_if_new_update():
    if settings.general.branch == "master":
        ...
        logging.error(
            "BAZARR unknown branch provided to updater: {}".format(
                settings.general.branch
            )
        )
        return
    logging.debug(
        "BAZARR updater is using {} branch".format(settings.general.branch)
    )

    check_releases()

    with open(
        os.path.join(args.config_dir, "config", "releases.txt"), "r"
    ) as f:
        data = json.load(f)
    if not args.no_update:
        if use_prerelease:
            ...
            release = next(
                (item for item in data if not item["prerelease"]), None
            )

        if release:
            logging.debug(
                "BAZARR last release available is {}".format(release["name"])
            )

            current_version = None
            try:
                current_version = semver.VersionInfo.parse(
                    os.environ["BAZARR_VERSION"]
                )
            ...
            if new_version and release["name"] != "v0.9.1.1":
                logging.debug(
                    "BAZARR newer release available and will be downloaded: {}".format(
                        release["name"]
                    )
                )
            ...
            # rolling back from nightly to stable release
                ...
                if current_version.prerelease and not use_prerelease:
                    logging.debug(
                        "BAZARR previous stable version will be downloaded: {}".format(
                            release["name"]
                        )
                    )
                    download_release(url=release["download_link"])
...
def download_release(url):
    ...
    try:
        ...
        logging.debug(
            "BAZARR unable to create update directory {}".format(update_dir)
        )
    else:
    ...
    if r:
        try:
            ...
            logging.exception(
                "BAZARR unable to download new release and save it to disk"
            )
        else:
            apply_update()
...
def apply_update():
    ...
    if os.path.isdir(update_dir):
        if os.path.isfile(bazarr_zip):
            logging.debug(
                "BAZARR is trying to unzip this release to {0}: {1}".format(
                    bazarr_dir, bazarr_zip
                )
            )
            try:
                with ZipFile(bazarr_zip, "r") as archive:
                    ...
                    if (
                        len(
                            {item.split("/")[0] for item in archive.namelist()}
                        )
                        == 1
                    ):
                    ...
                    for file in archive.namelist():
                        if (
                            file.startswith(zip_root_directory)
                            and file != zip_root_directory
                            and not file.endswith("bazarr.py")
                        ):
                            file_path = os.path.join(
                                bazarr_dir, file[len(zip_root_directory):]
                            )
                ...
                try:
                    logging.debug(
                        "BAZARR successfully unzipped new release and will now try to delete the leftover "
                        "files."
                    )
                    update_cleaner(
                        zipfile=bazarr_zip,
                        bazarr_dir=bazarr_dir,
                        config_dir=args.config_dir,
                    )
                except Exception:
                    logging.exception(
                        "BAZARR unable to cleanup leftover files after upgrade."
                    )
                else:
                    logging.debug(
                        "BAZARR successfully deleted leftover files."
                    )
            finally:
...
def update_cleaner(zipfile, bazarr_dir, config_dir):
    ...
    logging.debug(
        "BAZARR zip file contain {} directories and files".format(
            len(file_in_zip)
        )
    )
    ...
    if os.path.sep == "\\":
        logging.debug(
            "BAZARR upgrade leftover cleaner is running on Windows. We'll fix the zip file separator "
            "accordingly."
        )
        ...
        logging.debug(
            "BAZARR upgrade leftover cleaner is running on something else than Windows. The zip file "
            "separator are fine."
        )
    ...
    if os.path.abspath(bazarr_dir) in os.path.abspath(config_dir):
        dir_to_ignore.append(
            "^" + os.path.relpath(config_dir, bazarr_dir) + os.path.sep
        )
    dir_to_ignore_regex = re.compile("(?:% s)" % "|".join(dir_to_ignore))
    logging.debug(
        "BAZARR upgrade leftover cleaner will ignore directories matching this regex: "
        "{}".format(dir_to_ignore_regex)
    )

    file_to_ignore = ["nssm.exe", "7za.exe"]
    logging.debug(
        "BAZARR upgrade leftover cleaner will ignore those files: {}".format(
            ", ".join(file_to_ignore)
        )
    )
    extension_to_ignore = [".pyc"]
    logging.debug(
        "BAZARR upgrade leftover cleaner will ignore files with those extensions: "
    ...
    for foldername, subfolders, filenames in os.walk(bazarr_dir):
        relative_foldername = (
            os.path.relpath(foldername, bazarr_dir) + os.path.sep
        )
    ...
    logging.debug(
        "BAZARR directory contain {} files".format(len(file_on_disk))
    )
    logging.debug(
        "BAZARR directory contain {} directories".format(len(folder_list))
    )
    file_on_disk += folder_list
    logging.debug(
        "BAZARR directory contain {} directories and files".format(
            len(file_on_disk)
        )
    )
    ...
    logging.debug(
        "BAZARR will delete {} directories and files".format(
            len(file_to_remove)
        )
    )
    logging.debug(
        "BAZARR will delete this: {}".format(", ".join(file_to_remove))
    )
    ...
    for file in file_to_remove:
        ...
        try:
            ...
            logging.debug(
                "BAZARR upgrade leftover cleaner cannot delete {}".format(
                    filepath
                )
            )
