...
def store_subtitles(path, use_cache=True):
    ...
    if os.path.exists(path):
            ...
            try:
                ...
                for (
                    ...
                    try:
                        if (
                            (
                                settings.general.getboolean("ignore_pgs_subs")
                                and subtitle_codec.lower() == "pgs"
                            )
                            or (
                                settings.general.getboolean(
                                    "ignore_vobsub_subs"
                                )
                                and subtitle_codec.lower() == "vobsub"
                            )
                            or (
                                settings.general.getboolean("ignore_ass_subs")
            ...
            if dest_folder:
                if settings.general.subfolder == "absolute":
                    ...
                    full_dest_folder_path = os.path.join(
                        os.path.dirname(path), dest_folder
                    )
            subtitles = guess_external_subtitles(
                full_dest_folder_path, subtitles
            )
        except Exception:
        ...
        for episode in matching_episodes:
            if episode:
                logging.debug(
                    "BAZARR storing those languages to DB: "
...
def store_subtitles_movie(path, use_cache=True):
    ...
    if os.path.exists(path):
        ...
        try:
            ...
            core.CUSTOM_PATHS = [dest_folder] if dest_folder else []
            subtitles = search_external_subtitles(
                path, languages=get_language_set()
            )
            full_dest_folder_path = os.path.dirname(path)
            if dest_folder:
                if settings.general.subfolder == "absolute":
        ...
        ).where(TableMovies.path == path).execute()
        matching_movies = (
...
def list_missing_subtitles(no=None, epno=None, send_event=True):
    ...
    for episode_subtitles in episodes_subtitles:
            ...
            if cutoff_temp_list:
                for cutoff_temp in cutoff_temp_list:
                    ...
                    if cutoff_language in actual_subtitles_list:
                        ...
                        cutoff_language
                        and [cutoff_language[0], "True", "False"]
                        ...
                        cutoff_language
                        and [cutoff_language[0], "False", "True"]
                        in actual_subtitles_list
                    ):
                        cutoff_met = True
                        missing_subtitles_text = str([])
        ...
        TableEpisodes.update(
            {TableEpisodes.missing_subtitles: missing_subtitles_text}
        ).where(
            TableEpisodes.episodeId == episode_subtitles["episodeId"]
        ).execute()

        if send_event:
            event_stream(
                type="episode", payload=episode_subtitles["episodeId"]
            )
...
def list_missing_subtitles_movies(no=None, send_event=True):
    ...
    for movie_subtitles in movies_subtitles:
        ...
        TableMovies.update(
            {TableMovies.missing_subtitles: missing_subtitles_text}
        ).where(TableMovies.movieId == movie_subtitles["movieId"]).execute()
    ...
    for subtitle, language in subtitles.items():
        if not language:
            ...
            if (
                ...
                and os.path.splitext(subtitle_path)[1]
                in core.SUBTITLE_EXTENSIONS
            ):
                logging.debug(
