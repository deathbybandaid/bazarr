...
def blacklist_delete(provider, subs_id):
    TableBlacklist.delete().where(
        (TableBlacklist.provider == provider)
        and (TableBlacklist.subs_id == subs_id)
    ).execute()
...
def get_binaries_from_json():
    try:
        binaries_json_file = os.path.realpath(
            os.path.join(os.path.dirname(__file__), "binaries.json")
        )
...
def get_binary(name):
    ...
    if installed_exe and os.path.isfile(installed_exe):
        ...
        exe_dir = os.path.abspath(
            os.path.join(binaries_dir, system, machine, dir_name)
        )
        ...
        binary = next(
            (
                item
                for item in binaries_json
                if item["system"] == system
                and item["machine"] == machine
                and item["directory"] == dir_name
                and item["name"] == name
            ),
            None,
        )
        if not binary:
            ...
            logging.debug(
                "BAZARR found this in binaries.json: {}".format(binary)
            )

        if os.path.isfile(exe) and md5(exe) == binary["checksum"]:
            logging.debug(
                "BAZARR returning this existing and up-to-date binary: {}".format(
                    exe
                )
            )
            return exe
            ...
            try:
                logging.debug(
                    "BAZARR creating directory tree for {}".format(exe_dir)
                )
                os.makedirs(exe_dir, exist_ok=True)
                logging.debug(
                    "BAZARR downloading {0} from {1}".format(
                        name, binary["url"]
                    )
                )
                ...
                if system != "Windows":
                    logging.debug(
                        "BAZARR adding execute permission on {}".format(exe)
                    )
                    st = os.stat(exe)
                    os.chmod(exe, st.st_mode | stat.S_IEXEC)
            except Exception:
                logging.exception(
                    "BAZARR unable to download {0} to {1}".format(
                        name, exe_dir
                    )
                )
                raise BinaryNotFound
                ...
                logging.debug(
                    "BAZARR returning this new binary: {}".format(exe)
                )
                return exe
    ...
    def remove_expired(path, expiry):
        ...
        if mtime + datetime.timedelta(days=expiry) < now:
            try:
                ...
                logging.debug(
                    "Couldn't remove cache file: %s", os.path.basename(path)
                )
...
def delete_subtitles(
    ...
    if media_type == "series":
        try:
            ...
            logging.exception(
                "BAZARR cannot delete subtitles file: " + subtitles_path
            )
            ...
            store_subtitles(media_path)
            event_stream(
                type="episode-wanted", action="update", payload=episode_id
            )
            return True
    else:
        try:
            ...
            store_subtitles_movie(media_path)
            event_stream(
                type="movie-wanted", action="update", payload=movie_id
            )
            return True
...
def translate_subtitles_file(video_path, source_srt_file, to_lang, forced, hi):
    ...
    logging.debug(
        "BAZARR is translating in {0} this subtitles {1}".format(
    ...
    logging.debug(
        "BAZARR is sending {} blocks to Google Translate".format(
        ...
        try:
            translated_partial_srt_text = GoogleTranslator(
                ...
                target=language_code_convert_dict.get(
                    lang_obj.basename, lang_obj.basename
                ),
...
def check_credentials(user, pw):
    ...
    return (
        hashlib.md5(pw.encode("utf-8")).hexdigest() == password
        and user == username
    )
