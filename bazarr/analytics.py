...
def track_event(category=None, action=None, label=None):
    ...
    try:
        if os.path.isfile(
            ...
            visitor = pickle.loads(
                codecs.decode(visitor_text.encode(), "base64")
            )
    ...
    tracker.add_custom_variable(
        CustomVariable(
            index=1,
            name="BazarrVersion",
            value=os.environ["BAZARR_VERSION"].lstrip("v"),
            scope=1,
        )
    )
    tracker.add_custom_variable(
    ...
    tracker.add_custom_variable(
        CustomVariable(
            index=3, name="OSVersion", value=platform.platform(), scope=1
        )
    )
    ...
    try:
        ...
        with open(
            ...
            handle.write(
                codecs.encode(pickle.dumps(visitor), "base64").decode()
            )
