...
def load_language_in_db():
    ...
    langs = [
        [lang.alpha_3, lang.alpha_2, lang.name]
        for lang in pycountry.languages
        if hasattr(lang, "alpha_2")
    ]

    # Insert standard languages in database table
    TableSettingsLanguages.insert_many(
        langs,
        fields=[
    ...
    for lang in langs:
        TableSettingsLanguages.update(
            {TableSettingsLanguages.code3b: lang[0]}
        ).where(TableSettingsLanguages.code3 == lang[1]).execute()
...
def create_languages_dict():
    ...
    TableSettingsLanguages.update(
        {TableSettingsLanguages.name: "Chinese Simplified"}
    ).where(TableSettingsLanguages.code3 == "zho").execute()

    languages_dict = TableSettingsLanguages.select(
        TableSettingsLanguages.name,
        TableSettingsLanguages.code2,
        TableSettingsLanguages.code3,
...
def language_from_alpha2(lang):
    return next(
        (item["name"] for item in languages_dict if item["code2"] == lang[:2]),
        None,
    )
...
def language_from_alpha3(lang):
    return next(
        (
            ...
            if item["code3"] == lang[:3] or item["code3b"] == lang[:3]
        ),
        None,
    )
...
def alpha2_from_language(lang):
    return next(
        (item["code2"] for item in languages_dict if item["name"] == lang),
        None,
    )
...
def alpha3_from_language(lang):
    return next(
        (item["code3"] for item in languages_dict if item["name"] == lang),
        None,
    )
...
def get_language_set():
    languages = (
        TableSettingsLanguages.select(TableSettingsLanguages.code3)
