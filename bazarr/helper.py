...
def pp_replace(
    ...
    pp_command = pp_command.replace(
        "{{subtitles_language_code2_dot}}",
        str(language_code2).replace(":", "."),
    )
    pp_command = pp_command.replace(
        "{{subtitles_language_code3_dot}}",
        str(language_code3).replace(":", "."),
    )
    pp_command = pp_command.replace(
    ...
    if subfolder != "current" and fld_custom:
        ...
        if not os.path.isdir(fld):
            try:
                ...
                logging.error(
                    "BAZARR is unable to create directory to save subtitles: "
                    + fld
                )
                fld = None
    else:
