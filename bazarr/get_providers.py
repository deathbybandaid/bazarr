...
PROVIDER_THROTTLE_MAP = {
    ...
    "opensubtitlescom": {
        ...
        DownloadLimitExceeded: (
            datetime.timedelta(hours=hours_until_end_of_day),
            "{} hours".format(str(hours_until_end_of_day)),
        ),
    },
    "addic7ed": {
        DownloadLimitExceeded: (datetime.timedelta(hours=3), "3 hours"),
...
def get_providers_auth():
    return {
        ...
        "podnapisi": {"only_foreign": True, "also_foreign": False},
        "subscene": {
...
def get_throttled_providers():
    ...
    if os.path.exists(
        ...
        with open(
            os.path.normpath(
                os.path.join(
                    args.config_dir, "config", "throttled_providers.dat"
                )
            ),
            "r",
        ) as handle:
