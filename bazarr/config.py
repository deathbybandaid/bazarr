...
defaults = {
    ...
    "opensubtitlescom": {"username": "", "password": "", "use_hash": "True"},
    ...
    "ktuvit": {"email": "", "hashed_password": ""},
    "legendastv": {"username": "", "password": "", "featured_only": "False"},
    "xsubs": {"username": "", "password": ""},
    "assrt": {"token": ""},
    "napisy24": {"username": "", "password": ""},
    "subscene": {"username": "", "password": ""},
    "betaseries": {"token": ""},
    "titlovi": {"username": "", "password": ""},
    "titulky": {"username": "", "password": ""},
}
...
settings.general.base_url = (
    settings.general.base_url if settings.general.base_url else "/"
)
...
def save_settings(settings_items):
    ...
    for key, value in settings_items:
        ...
        if key in [
            "settings-series-excluded_tags",
            "settings-series-only_monitored",
            "settings-series-excluded_series_types",
            "settings.movies.excluded_tags",
    ...
    if settings.proxy.type != "None":
        if settings.proxy.username != "" and settings.proxy.password != "":
            proxy = (
                settings.proxy.type
                + "://"
                + quote_plus(settings.proxy.username)
                + ":"
                + quote_plus(settings.proxy.password)
                + "@"
                + settings.proxy.url
                + ":"
                + settings.proxy.port
