...
def update_notifier():
    ...
    notifiers_to_delete = [
        item for item in notifiers_current if item not in notifiers_old
    ]
    ...
    for item in notifiers_to_delete:
        TableSettingsNotifier.delete().where(
            TableSettingsNotifier.name == item
        ).execute()
...
def get_notifier_providers():
    providers = (
        TableSettingsNotifier.select(
            TableSettingsNotifier.name, TableSettingsNotifier.url
        )
...
def get_series(series_id):
    data = (
        TableShows.select(TableShows.title, TableShows.year)
...
def get_movie(movie_id):
    data = (
        TableMovies.select(TableMovies.title, TableMovies.year)
